...
const rule: Rule.RuleModule = {
  ...
  create(context) {
    ...
    const checkHandlerBody = ({
      ...
      if (allowIsComposingGuard && containsEnterKeyCheckOutsideIsComposingGuard({ node: body, eventParamName }) === false) {
      ...
      if (allowIsComposingGuard && guardFunctions.length > 0 && containsEnterKeyCheckOutsideIsComposingGuard({ node: body, eventParamName, guardFunctions }) === false) {
        return;
      }
      if (allowIsComposingGuard && containsEnterKeyCheckOutsideModifierGuard({ node: body, eventParamName }) === false) {
        return;
      }
      if (containsEnterKeyCheck({ node: body, eventParamName })) {
        context.report({
          node: reportNode,
