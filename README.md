...
| [`require-ime-safe-input`](#require-ime-safe-input) | Flags `event.target.value` rewrites in `input`/`beforeinput`/JSX `onChange` handlers without an `e.isComposing` guard | ❌ (not yet — monitoring for noise) |
...
Detects `event.target.value` / `event.currentTarget.value` rewrites in `input`/`beforeinput` handlers (and JSX `onChange`) that lack an `e.isComposing` guard — rewriting the value mid-composition destroys the in-progress IME conversion. Not part of `recommended` yet; it's shipped opt-in while real-world usage is checked for false positives.

See the [full rule documentation](https://github.com/hiroya-uga/eslint-plugin-ime-safe-form/blob/main/docs/rules/require-ime-safe-input.md) for known limitations.

#### Detected patterns

- `element.addEventListener('input' | 'beforeinput', handler)` rewriting `event.target.value` / `event.currentTarget.value` **without** an `e.isComposing` guard
- `element.oninput` / `element.onbeforeinput` assignments (native `onchange` is out of scope)
- JSX `onInput` / `onBeforeInput` / `onChange` props on IME-capable elements
- Destructured event parameters, e.g. `({ target, isComposing }) => { ... }`
